...
contract Token {
    /* 这是一个轻微的变化对erc20基础标准。
      功能totalsupply()收益不变（uint256供应）；
      被替换为：
       uint256公共供应总量；
        这会自动创建一个getter函数供应总量。
        由于公共吸气剂函数没有，所以将它移到基本契约。
       目前被认为是匹配抽象的实现
       编译器的功能。
    */
...
contract DentacoinToken is owned, SafeMath, StandardToken {
    string public name = "BliBli";
    string public symbol = "BliBli";
    address public DentacoinAddress = this;
    uint8 public decimals = 0;
    uint256 public totalSupply = 80000000;
    uint256 public buyPriceEth = 1000 finney;                                  // Buy price for Dentacoins
    uint256 public sellPriceEth = 1000 finney;                                 // Sell price for Dentacoins
    uint256 public gasForDCN = 5000 finney;                                    // Eth from contract against DCN to pay tx (10 times sellPriceEth)
